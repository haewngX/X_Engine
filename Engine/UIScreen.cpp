...
void UIScreen::ProcessInput(const class InputState* state)
{
	// Do we have buttons?
	if (!mButtons.empty())
	{
		...
		float x, y;
		mGame->GetInputSystem()->GetState()->Mouse.GetMousePosition(mGame->GetRenderer()->GetWindow(),&x,&y);
...
void UIScreen::HandleKeyPress(const class InputState* state)
{
	if (state->Mouse.GetButtonState(0) == EPressed)
	{
		if (!mButtons.empty())
		{
			for (auto b : mButtons)

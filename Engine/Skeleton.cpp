...
bool Skeleton::Load(const std::string& fileName)
{
	...
	if (!file.is_open())
	{
		printf("File not found: Skeleton %s", fileName.c_str());
		return false;
	}
	...
	if (!doc.IsObject())
	{
		printf("Skeleton %s is not valid json", fileName.c_str());
		return false;
	}
	...
	if (ver != 1)
	{
		printf("Skeleton %s unknown format", fileName.c_str());
		return false;
	}
	...
	if (!bonecount.IsUint())
	{
		printf("Skeleton %s doesn't have a bone count.", fileName.c_str());
		return false;
	}
	...
	{
		printf("Skeleton %s exceeds maximum bone count.", fileName.c_str());
		return false;
	}
	...
	if (!bones.IsArray())
	{
		printf("Skeleton %s doesn't have a bone array?", fileName.c_str());
		return false;
	}

	if (bones.Size() != count)
	{
		printf("Skeleton %s has a mismatch between the bone count and number of bones", fileName.c_str());
		return false;
	}

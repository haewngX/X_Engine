...
bool Animation::Load(const std::string& fileName)
{
	...
	if (!file.is_open())
	{
		printf("File not found: Animation %s", fileName.c_str());
		return false;
	}
	...
	if (!doc.IsObject())
	{
		printf("Animation %s is not valid json", fileName.c_str());
		return false;
	}
	...
	if (ver != 1)
	{
		printf("Animation %s unknown format", fileName.c_str());
		return false;
	}
	...
	{
		printf("Animation %s doesn't have a sequence.", fileName.c_str());
		return false;
	}
	...
	{
		printf("Sequence %s has invalid frames, length, or bone count.", fileName.c_str());
		return false;
	}
	...
	if (!tracks.IsArray())
	{
		printf("Sequence %s missing a tracks array.", fileName.c_str());
		return false;
	}
	...
	for (rapidjson::SizeType i = 0; i < tracks.Size(); i++)
	{
		...
		{
			printf("Animation %s: Track element %d is invalid.", fileName.c_str(), i);
			return false;
		}
		...
		{
			printf("Animation %s: Track element %d is missing transforms.", fileName.c_str(), i);
			return false;
		}
		...
		{
			printf("Animation %s: Track element %d has fewer frames than expected.", fileName.c_str(), i);
			return false;
		}
		...
		for (rapidjson::SizeType j = 0; j < transforms.Size(); j++)
		{
			...
			{
				printf("Skeleton %s: Bone %d is invalid.", fileName.c_str(), i);
				return false;
			}
